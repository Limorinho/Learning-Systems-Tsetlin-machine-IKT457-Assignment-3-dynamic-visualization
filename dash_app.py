...
def update_chart(s, p_l_given_y, p_y, p_not_l_given_not_y):
    ...
    colors = ['lightblue'] * 6 + ['mediumpurple'] * 2
    ...
    # Add annotations
    annotations = []
    if max(distribution[:6]) > 0:
        annotations.append(dict(
            x=3, y=max(distribution[:6]) + max(distribution) * 0.1,
            text="States 1-6", showarrow=False,
            font=dict(size=12, color="gray")
        ))
    if max(distribution[6:]) > 0:
        annotations.append(dict(
            x=7.5, y=max(distribution[6:]) + max(distribution) * 0.1,
            text="States 7-8", showarrow=False,
            font=dict(size=12, color="gray")
        ))
    
    fig.update_layout(
        title='Stationary Distribution - Literal Automaton (8 States)',
        xaxis_title='State',
        yaxis_title='Probability',
        xaxis=dict(
            tickmode='array',
            tickvals=list(range(1, 9)),
            ticktext=[f'π{i}' for i in range(1, 9)]
        ),
...
if __name__ == '__main__':
    ...
    print("\nTesting with sample values (s=5, P(L|Y)=0.8, P(Y)=0.6, P(~L|~Y)=0.7):")
    test_dist = calculate_stationary_distribution(5.0, 0.8, 0.6, 0.7)
    print(f"Distribution: {test_dist}")
    print(f"Sum: {np.sum(test_dist):.6f}")
    print(f"States 7-8: π₇={test_dist[6]:.4f}, π₈={test_dist[7]:.4f}")
